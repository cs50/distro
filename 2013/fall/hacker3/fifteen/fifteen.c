 ...
 * The tiles are placed in a pseudorandom, yet solvable, configuration.
 */
void
init(void)
{
    ...
    // if the pseudorandom configuration of the puzzle is not solvable, just
    // swap the top-left two values
    if(!winnable_config())
    {
        int temp = board[0][0];
        board[0][0] = board[0][1];
 ...
 */
void
slide(int delta_col_0,int delta_row_0)
{
...
/*
 * move_toward puts the tile to the left of the blank into correct row and col
 */
void
move_toward(int correct_row, int correct_col)
{
 ...
 */
void
locate(int tile, int* row, int* col)
{
    for(int i = 0; i < d; i++)
        for(int j = 0; j < d; j++)
            if(board[i][j]==tile)
...
/*
 * arrange_blank moves the blank so that it is to the right of the tile
 * that we are working on.
 */
void
arrange_blank(int tile,int row, int col)
{
 ...
 */
int
brute_helper(int tile, int solution_path[], int steps)
{
...
int
brute_force(int solution_path[32], int steps)
{
    int total_steps;
...
/*
 * god_mode is the function that will ultimately solve the board. It works by
...
god_mode(void)
{
    ...
    for(int corner = 0, tile, row, col, correct_row, correct_col; 
        corner < d-3; corner++)
    { 
